...
# -----------------------------------------
# REAL BROWSER-BASED GEOLOCATION (ACCURATE)
# -----------------------------------------
def get_browser_location():
    ...
    st.components.v1.html(geocode_js, height=0)

    location = st.session_state.get("BROWSER_LOCATION", None)
    return location

# Receive browser location via postMessage
...
# Listener for browser → Python messages
def setup_location_listener():
    msg = st.experimental_get_query_params()
    return
...
# ----------------------------------------------------------
# STEP 1: Try REAL browser-based location
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# STEP 2: Fallback to IP detection if browser geolocation not allowed
# ----------------------------------------------------------
if user_city_actual:
    auto_city_raw = user_city_actual
else:
    auto_city_raw = detect_user_city()   # your original IP method
...
with col1:
    use_auto = st.checkbox(f"📍 Auto-detect ({auto_city_raw})", value=False)
with col2:
