...
# --------------------------------------------------------------
# ⭐⭐ FIXED WORKING GEOLOCATION (Streamlit Cloud compatible)
# --------------------------------------------------------------
...
def get_browser_location():
    ...
    js_code = """
        <script>
            ...
            function sendLocation(lat, lon) {
                streamlitWindow.postMessage({location: {lat: lat, lon: lon}}, "*");
            }

            function requestLocation() {
                if (!navigator.geolocation) {
                    streamlitWindow.postMessage({location: null}, "*");
                    return;
                }
    ...
    msg = st.experimental_get_last_message()

    if msg and "location" in msg:
        return msg["location"]
...
# --------------------------------------------------------------
# GET BROWSER OR IP LOCATION
# --------------------------------------------------------------

browser_location = get_browser_location()
user_city_actual = None

if browser_location and browser_location.get("lat") and browser_location.get("lon"):
    try:
