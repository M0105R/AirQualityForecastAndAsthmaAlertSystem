# --------------------------------------------------------------
# app.py  (with working scroll navigation + in-app notifications)
# (Modified: fixed browser location implementation + defined auto_city_matched)
# --------------------------------------------------------------
...
# --------------------------------------------------------------
#  UTIL + ORIGINAL FUNCTIONS  (same as before)
# --------------------------------------------------------------
...
# ----------------------------------------------------------
# STEP 1: Try REAL browser-based location (bridge via hidden text_input)
# ----------------------------------------------------------

def get_browser_location():
    # Invisible input to store coordinates
    loc_val = st.text_input(
        key="browser_location_state",
        label="",
    ...
    # Completely hide the input from DOM
    st.markdown("""
        <style>
            input#browser_location_state {
    ...
    # JS to request location and write result to hidden input
    st.markdown("""
        <script>
        const sendLocation = () => {
            if (!navigator.geolocation) {
                const payload = {error:"no_support"};
                const input = window.parent.document.querySelector('input#browser_location_state');
    ...
    # Read parsed value from session state
    raw = st.session_state.get("browser_location_state", "")
    if not raw:
        return None
...
# STEP 2: Reverse geocode if browser location works
browser_location = get_browser_location()

user_city_actual = None
...
if browser_location and "lat" in browser_location:
    try:
        ...
        geo_res = requests.get(geocode_url, headers={"User-Agent": "Mozilla/5.0"}).json()

        if "address" in geo_res:
            user_city_actual = geo_res["address"].get("city") or \
                               geo_res["address"].get("town") or \
                               geo_res["address"].get("village")
    except:
        user_city_actual = None

# FALLBACK to IP
if user_city_actual:
    auto_city_raw = user_city_actual
else:
    auto_city_raw = detect_user_city()

# Best match
auto_city_matched = None
if auto_city_raw and cities:
    auto_city_matched = find_best_city_match(auto_city_raw, cities)
...
if use_auto and auto_city_matched:
    city = auto_city_matched
    try:
        st.toast(f"📍 Auto-detected city: {auto_city_raw}", icon="📍")
    except:
        st.info(f"Auto-detected city: {auto_city_raw}")
elif use_auto and auto_city_raw and not auto_city_matched:
    city_guess = find_best_city_match(auto_city_raw, cities)
    if city_guess:
        city = city_guess
        try:
            st.toast(f"📍 Auto-detected: {auto_city_raw} → using {city}", icon="📍")
        except:
            st.info(f"Auto-detected: {auto_city_raw} → using {city}")
    else:
        city = auto_city_raw
        try:
            st.toast(f"⚠️ City not in list, using detected: {auto_city_raw}", icon="⚠️")
        except:
            st.warning(f"City not in list, using detected: {auto_city_raw}")
else:
    city = selected_city
